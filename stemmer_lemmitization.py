...
stemmer_sentences = nltk.sent_tokenize(data)
...
## Stemmer technique
for i in range(len(stemmer_sentences)):
    # Separates the words from each sentences.
    stemmer_words = nltk.word_tokenize(stemmer_sentences[i])
    ...
    words = [stemmer.stem(word) for word in stemmer_words if word not in set(stopwords.words('english'))]
    stemmer_sentences[i] = ' '.join(words)

print('result after stemmer technique on text data')
for i in range(len(stemmer_sentences)):
    print(stemmer_sentences[i])

## Lemmatization technique
# Creates the separate sentences out of entire data.
lemmatization_sentences = nltk.sent_tokenize(data)
...
## Stemmer technique
for i in range(len(lemmatization_sentences)):
    # Separates the words from each sentences.
    lemmatization_words = nltk.word_tokenize(lemmatization_sentences[i])
    # Applying stemming on each words after removing words
    # which does not add values to the data by using stopwords() which is available in various languages.
    words = [lemmatizer.lemmatize(word) for word in lemmatization_words if word not in set(stopwords.words('english'))]
    lemmatization_sentences[i] = ' '.join(words)

print('result after lemmatization technique on text data')
for i in range(len(lemmatization_sentences)):
    print(lemmatization_sentences[i])
